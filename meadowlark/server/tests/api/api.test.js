...
describe('API tests', () => {
	...
	expect(vacation[0].name).toBe(vacation0.name)
    })
	...
	expect(vacations.length).not.toBe(0)
	const vacation0 = vacations[0]
	//at this moment, all we can do is make sure the HTTP requrest is successful
	await _fetch('delete', `/api/vacation/${vacation0.sku}`, { email: 'test@meadowlarktravel.com', notes: '' })
    })
})
